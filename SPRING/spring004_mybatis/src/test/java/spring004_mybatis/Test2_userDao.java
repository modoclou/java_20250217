...
public class Test2_userDao {
	...
	@Ignore /*@Test*/ public void insert() {
		...
		System.out.println(dao.insert(dto));	//1 - 성공
	}
	@Ignore /*@Test*/ public void selectOne() {
		System.out.println(dao.selectOne(1));	//1 - 성공
	}//UserDto [no=1, name=first, age=11]
	
	@Ignore /*@Test*/ public void update() {
		UserDto dto = new UserDto();
		dto.setName("five");	dto.setAge(5);	dto.setNo(5);	//있는 번호로 수정했는지 확인
		System.out.println(dao.update(dto));	//1 - 성공
		System.out.println(dao.selectOne(5));	//UserDto [no=5, name=five, age=55]
	}
