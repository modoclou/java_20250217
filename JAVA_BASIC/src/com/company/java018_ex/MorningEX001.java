...
public class MorningEX001 {
	public static void main(String []args) {
		System.out.println(" 다음 중 인스턴스변수의 초기화 순서를 적으시오.  ");   //#### static 초기화 > 지역변수 초기화 > 할당된 값 촉;화
		Test t1 = new Test();  System.out.println(t1.iv);				//2번째?
	   
	} // end main
} // end class

/*
runtime area [  method  | heap | stack | pc register | native method stack]
...
public class ExtendsBasic009_2_Polymorphism2_short {
	...
	TestB2   tb = ta;                  // [ tb   [b]-[a]  ]
	// 오류나는 이유는? 
	//Type mismatch: cannot convert from MilkPrint.TestA2 to MilkPrint.TestB2
		}
	}
}
/*

Q6.    1. 콜렉션프레임워크의 종류 (필기)
		 List   :    순서 [] , 중복허용 [O]  
				=>  사용가능메서드: [추가][삭제][갯수][데이터꺼내기] | add.delete.size.
		 Set    :    순서 [] , 중복허용 [X]  
				=>  사용가능메서드: [추가ht][삭제delete][갯수size][데이터꺼내기]
		 Map    :    순서가아닌 [ , ]의 쌍(ENTRY)으로 이루어짐
		                =>  사용가능메서드: [추가put][삭제delete][갯수size][데이터꺼내기]  


Q7.  ArrayList
 		 * 	 dto 를 이용하여 ArrayList + Iterator  이용해서 만들기  
	List 
	add.get(new ArrayList(", int))
Q8.   HashSet
