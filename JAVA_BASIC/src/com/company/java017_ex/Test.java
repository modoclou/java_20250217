...
> 클래스(설계도)인스턴스화를 통해 객체를 만든 것(Instance)
> 인스턴스 heap 에어리어의 실제메모리를 객체화
class: 계속 사용
instance: 일회성... 
...
class A{
    ...
    void   show2() {             System.out.println(Company);  } 
...
인스턴스변수			:    (  a  )   ,  		메모리위치 영역 -  (   stack    )    area
지역변수				:    (  method  )   ,  	메모리위치 영역 -   (   heap   )   area

8-2. 오류가 날만한 위치는?
void   show() {             
System.out.println(this.a);  }
 Stack은 수동으로 초기화를 해야한다.
...
/*
------------------------------------------------------------------------------------------------
클래스 - 상속x / 멤버변수 - 상수 멤버함수 - 오버라이딩 X
9-1.  오류나는 부위를 적으시오.
		FinalBasic / show() / name
		@Override void show() {  System.out.println("HELLO~" +  name); }
		
9-2.  오버로딩과 오버라이딩의 차이점은?
		오버로딩: 비슷한 기능을 하는 메서드들의 이름을 갈게 하고 알규몬트의 자료형과 갯수로 구분하는 것.
